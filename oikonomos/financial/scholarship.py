...
*award* (`Scholarship Award`) — both oikonomos doctypes. Discounts are computed
at invoice time (``oikonomos.financial.billing.resolve_scholarship``); this module
owns everything around the award lifecycle that is *not* invoice construction: how
much budget is left, what the portal may offer, granting a request, and the daily
retention review.

Relocated from seminary (which is Frappe-only and owns no scholarship logic). The
student SPA still calls three of these by their seminary path; those survive in
`seminary.seminary.scholarship` as thin shims that delegate here through the
financial backend. The daily retention review is registered as an oikonomos
scheduler task.
"""
...
# Scholarship Award workflow primitives (canonical copies for the billing engine
# live in oikonomos's scholarship_award.py controller).
ACTIVE_STATE = "Active"
...
# --------------------------------------------------------------------------
# Retention review (daily scheduler — registered in oikonomos hooks.py)
# --------------------------------------------------------------------------
