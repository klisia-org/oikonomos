...
def on_seminary_settings_update(doc, method=None):
    """Seminary Settings on_update (subscribed from oikonomos). When instructor
    payroll is enabled, provision the Salary Slip custom fields + Instructor Pay
    component. Relocated from the seminary Seminary Settings override so payroll
    setup is owned by the bridge."""
    if not getattr(doc, "hrms_enable", None):
        return
...
def provision_payroll_if_enabled():
    """Idempotent payroll provisioning for migrate/install. No-op unless HRMS is
    installed and instructor payroll is enabled in Seminary Settings."""
    if not _hrms_enabled():
        return
    ensure_custom_fields()
    ensure_salary_components()
